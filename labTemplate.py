...
def makeGrayscalePython(img):
	...
	for i in range(0, w): # loop through every pixel in width
		for j in range(0, h): # loop through every pixel in height
			...
			g = int(img[j][i][1])
			r = int(img[j][i][2])
			gray[j][i][0] = -1 # calculate average here
	end = time.time()
...
def bgrThreshold(img):
	lowerThresh = np.array([-1,-1,-1]) # change threshold values
	upperThresh = np.array([-1,-1,-1]) # change threshold values
...
def detectCanny(img):
	thresh1 = -1 # change threshold
	thresh2 = -1 # change threshold

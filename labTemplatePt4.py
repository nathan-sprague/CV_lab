...
def train():
	...
	model.export(export_dir='.', tflite_filename='trained_models/queen_worker.tflite')


def setUpDetector():
	base_options = core.BaseOptions(file_name="trained_models/queen_worker.tflite", use_coral=False, num_threads=4)
...
def detectQueenWorkers(img, detector):
	...
	for i in detection_result.detections:
		...
		color = (0,255,0)
		if (i.categories[0].category_name) == "worker":
			color = (255,0,0)
...
def run():
	...
	if partNum == 0:
		...
		video="bee_images/a_queen_IMG_2234.mov"
		...
		while True:
			...
			if ret:	
				...
				detected = detectQueenWorkers(img.copy(), detector)
